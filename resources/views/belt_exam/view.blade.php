...
<section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{__('web.Belt Exam Members')}}</h1>
          </div>
        </div>
      </div>
</section>
  	...
  	<section class="content">
    	<div class="container-fluid">
              ...
              <br>
                ...
                <form method="post" action="{{route('register.filter-belt-exam')}}">
              <div class="row">
              @csrf
            ...
            <div class="card">
                ...
                <table class="table table-striped CommonDataTables" id="">
                  <thead>
                    <tr>
                      <th width="2%">#</th>
                      <th width="8%">{{__('web.Belt Exam Name')}}</th>
                      <th width="8%">{{__('web.Member Name')}}</th>
                      <th width="6%">{{__('web.Sports')}} | {{__('web.Coach')}}</th>
                      <th width="4%">{{__('web.Age')}}</th>
                      <th width="5%">{{__('web.Current Belt')}}</th>
                      <th width="5%">{{__('web.Next Belt')}}</th>
                      <th width="10%">{{__('web.Exam Date')}}</th>
                      <th width="10%">{{__('web.Result Date')}}</th>
                  ...
                  @foreach($belt_exam as $key)  
                    <tr>
                      ...
                      <td>{{$key->current_belt_name->name}}</td>
                      <td>{{$key->next_belt_name->name}}</td>
                      <td>{{$key['exam_date']}}</td>
                  ...
                  <tfoot>
                    <tr style="background: slategray; color: white; font-size: 18px;">
                      <td colspan="9" align="right"><b>{{__('web.Total')}}</b></td>
                      <td>{{$total_amount}}</td>
                      <td>{{$total_paid}}</td>
