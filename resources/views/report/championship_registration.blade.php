...
<section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{__('web.Championship Registration')}}</h1>
          </div>
        </div>
      </div>
</section>
  	...
  	<section class="content">
    	<div class="container-fluid">
              ...
              <form method="post" action="{{route('reports.filter-championship-registration')}}">
              <div class="row">
              @csrf
                ...
                <table class="table table-striped CommonDataTables" id="">
                  <thead>
                    <tr>
                      ...
                      <th width="20%">{{__('web.Championship Date')}}</th>
                      <th width="15%">{{__('web.Total Amount')}}</th>
                      <th width="15%">{{__('web.Paid Amount')}}</th>
                      <th width="18%">{{__('web.Remaining Amount')}}</th>
                      <th width="20%">{{__('web.Added on')}}</th>
                      <th style="text-align: center;" width="5%">{{__('web.Action')}}</th>
                    </tr>
                  </thead>
                  <tbody>
                     <?php
                  ...
                  @foreach($championship as $key)  
                    <tr>
                      ...
                      <td><b><a href="{{route('register.details-member',[$key['member_id']])}}">{{$key->member_info->name}}</a></b></td>
                      ...
                      <td style="text-align: center;"><a title="{{__('web.Details')}}" data-toggle="tooltip" onclick='ShowChampionshipDetails("{{$key['id']}}")' href="javascript:void(0)"><i class="fa fa-eye text-secondary"></i></a></td>
                    </tr>
                    <?php
